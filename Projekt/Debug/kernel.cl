...
void pbkdf2(unsigned char *password, unsigned char *salt, unsigned int c, unsigned int dkLen, unsigned int pass_length, unsigned int salt_length, global unsigned char *T)
{
    unsigned int l;
...
__kernel void func(__global unsigned char *haslo, __global unsigned char *sol, __global unsigned int *c, __constant unsigned int *dkLen, __global unsigned int *pass_length, __global unsigned int *salt_length, __global unsigned char *T)
{
	...
	for(i=0;i<pass_length[0];i++) password[i]=haslo[i];
	unsigned char salt[256];
	for(i=0;i<salt_length[0];i++) salt[i]=sol[i];

    pbkdf2(password, salt, c[0], dkLen[0], pass_length[0], salt_length[0], T);
